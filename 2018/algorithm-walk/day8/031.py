...
4. 각 도시 이름은 공백, 숫자, 특수문자 등이 없는 영문자로 구성되며, 대소문자 구분을 하지 않는다.
    ...
    if cacheSize:
        for i, city in enumerate(cities):
            ...
            if city in cache:
                ...
                calledCache.remove(index)
                calledCache.append(index)
                result = result + 1
            else:
                index = len(calledCache) if len(calledCache) < cacheSize else calledCache.pop(0)
...
# 해설 (코딩도장)
# 나와의 차이 : 나처럼 index list를 따로 구현하는게 아니라, cache에 우겨넣고 길이를 유지함
def measure(size, cities):
    cache =[''] * size

    time = 0
    for city in cities:
        if city.lower().strip() in cache:
            time += 1
        else:
            time += 5
            cache.append(city.lower().strip())
            cache.pop(0)

    return time

print(measure(cacheSize, cities))